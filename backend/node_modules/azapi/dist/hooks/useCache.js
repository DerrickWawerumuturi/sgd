...
var _MemoryCacheStrategy = _interopRequireDefault(require("../cachingStrategies/MemoryCacheStrategy"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function useCache(name, defaultValue, {
  strategy,
  ...options
} = {}) {
  if (!strategy) {
    strategy = (0, _MemoryCacheStrategy.default)(options);
  }
...
function createItem(defaultValue, options, prev) {
  const item = typeof defaultValue === 'function' ? defaultValue(options) : {
    value: defaultValue
  }; // not change
...
function isExpire({
  ...
  let expiredOn; // expire after ms

  if (typeof expire === 'number') {
    expiredOn = (date instanceof Date ? date.getTime() : new Date(date).getTime()) + expire;
